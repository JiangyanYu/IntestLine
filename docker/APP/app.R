          ...
          "
               ...
               fluidRow(
                 ...
                 column(width = 12,
                        offset = 4,
                        actionButton(
                          "selectArea", "1.5. Select Area", icon = icon("chart-area"),
...
## panel upper right: select backbone ----   
  column( width = 6,
          wellPanel(
            ...
            h5(HTML(
              paste(
                tags$b(span("Please selecte backbone points from proximal to distal in order. The order you select your points will be used to order your base layer. You can skip this step if you have uploaded one outline file and go to step 2.5.", style = "color:#993333;font-size:18;")),
            ...
            #hr(),
            plotOutput("plotSelectBackbone", click = "clicked",height = "550px",width = "100%"),
            br(),
           fluidRow( column(
              width = 4,
              offset = 4,
           ...
           fluidRow(
             column(width = 4,
                    offset = 4,
                    actionButton("backbonePlotButton", "2.5. Plot backbone on ROI",
           ...
           # hr(),
            ...
            fluidRow(
              column(
                width =4,
                offset = 4,
                actionButton("qualityControl", "QC of backbone selection",  icon = icon("clipboard-check"),
      ...
      if (isTruthy(upload_backbone)) {
        ...
        if(nrow(backbone_points)==0|nrow(backbone_points)==1){
          ...
          output$backbonePlot = renderPlot({
            ...
            points(backbone_points$x,backbone_points$y,pch=16,col="red",cex = 0.5)
            showNotification("Use currently selected backbone points.", type = "warning")
          })
        }
      }
...
## Overlay markers ----    
    output$marker_plot = renderPlot({
      ...
      marker_overlay = merge(plot_linear, input_image, by = "pos", no.dups = T, suffixes = c("","1"))
      ...
      ggplot(NULL)+
        ...
        ylab("Thickness (Serosa-luminal axis)")+xlab("Length (Proximal/outer-distal/inner axis)")+
